# Magnolia

Magnolia CMS and its ecosystem.

## Magnolia CLI

## Tutorials (v6.4)

- Content App: [My first content app](https://docs.magnolia-cms.com/product-docs/apps/developing-an-app/my-first-content-app/)
- Headless CMS: [Hello Magnolia - Headless](https://docs.magnolia-cms.com/product-docs/developing/headless/getting-started-with-magnolia-headless/hello-headless/)
- Actions in apps, forms and dialogs: [Action definition](https://docs.magnolia-cms.com/product-docs/apps/app-configuration/action-definition)
- Multisite (ony one site is allowed in the Community Edition): [How to use Multisite](https://docs.magnolia-cms.com/product-docs/developing/templating/site-definition/how-to-use-multisite/)
